...
import { fetchKpis, fetchTrend, scopeFromParams } from '@/lib/reports/queries';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Defaults to employees who still work here; ?includeInactive=1 widens it.
    ...
    const [kpis, trend] = await Promise.all([fetchKpis(scope), fetchTrend(12, scope)]);
